...
public sealed class PaymentWindow : Window, IComponentConnector
{
	private bool _activated;

	private PaymentPage _page;
	...
	{
		InitializeComponent();
		((Window)this).Activated += PaymentWindow_Activated;
	}

	private void PaymentWindow_Activated(object sender, WindowActivatedEventArgs args)
	{
		try
		{
			if (!_activated)
			{
				...
				((Window)this).AppWindow.Title = Loc.Get("PaymentWindowTitle");
				((Window)(object)this).SetDialogProperties(450, 700, resizable: true);
				_page = new PaymentPage();
		...
		{
			Log.Error(ex, "Error activating payment window");
		}
	}
	...
	public void InitializeComponent()
	{
		...
		{
			_contentLoaded = true;
			Uri uri = new Uri("ms-appx:///AppWindows/PaymentWindow.xaml");
